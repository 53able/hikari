...
import { createHikariHarness, type PiTurnHistoryMessage } from '../../src/pi.js';
import {
  listBooks,
  getBook,
  purchaseBook,
...
const { runTurn } = createHikariHarness({
  registry,
  auditLog,
...
const conversationHistory: PiTurnHistoryMessage[] = [];

const runPrompt = async (userMessage: string): Promise<void> => {
  console.log(`User: ${userMessage}\n`);

  const historyForTurn = [...conversationHistory];
