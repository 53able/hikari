...
# write / financial（idempotencyKey は invoker が入力から自動生成）
npx tsx examples/bookstore/dev-invoke.ts add_book '{"title":"Demo","author":"Tester","price":12.5,"stock":2}' admin
```

`write` / `financial` も invoker から実行できます（同一 user・capability・input なら同じ冪等キー）。HTTP クライアントは引き続き `Idempotency-Key` ヘッダの明示送信が必要です。
...
Pi / Claude / OpenAI チャット経路では、write/financial ツール実行時に `{traceId}:{toolCallId}` 形式の `idempotencyKey` をアダプターが自動付与します（HTTP API はクライアントが `Idempotency-Key` を送る必要があります）。
